---
title: "Backend Mülakat NodeJS"
description: "İş görüşmesinde sorulan bir backend sorusu ve cevabım."
image: "../../public/blogs/word-search-java.png"
...
# İş Görüşmesi Mülakat Sorusu ve Cevabı(Nodejs)
...
```js title="index.js" caption="Nodejs mülakat cevabı"
const fs = require("fs");
...
async function start() {
  ...
  fs.stat(filePath, (err, stats) => {
    ...
    function writeData() {
      ...
      const wordText =
        wordJson.word + " : " + wordJson.meanings.toString() + "\n";
      ...
      writeStream.write(wordText, (err) => {
        ...
        if (writeStream.bytesWritten < totalFileSizeInBytes - fileSizeInBytes) {
          ...
          writeStream.end(() => {
            ...
            console.log(
              `\n-> Yazma işlem süresi: ${Math.floor(
                elapsedTime / 1000 / 60 / 60
              )} saat ${Math.floor(elapsedTime / 1000 / 60)} dakika  ${
                Math.floor(elapsedTime / 1000) % 60
              } saniye. Toplam Milisaniye: ${elapsedTime}`
            );
...
function readText() {
  ...
  rl.on("close", () => {
    ...
    console.log(
      `\n-> OkumaYazma işlem süresi: ${Math.floor(
        elapsedTime / 1000 / 60 / 60
      )} saat ${Math.floor(elapsedTime / 1000 / 60)} dakika  ${
        Math.floor(elapsedTime / 1000) % 60
      } saniye. Toplam Milisaniye: ${elapsedTime}`
